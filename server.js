    ...
    for (let k in entryData) {
        ...
        if (k === "featured") {
            ...
            for (let k2 in entryData[k]) {
                entryData[k][k2] = entryData[k][k2][0].split(",");
                for (let i = 0; i < entryData[k][k2].length; i++) {
                    entryData[k][k2][i] = entryData[k][k2][i].trim();
                    if (entryData[k][k2][i] === "") {
                        throw new Error("Empty passes:" + k2 + ", maybe a trailing comma? " + url);
                    }
                }
            }
        }
